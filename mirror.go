...
func (m *Mirror) fetchAndApply(ctx context.Context, remote RemoteSpec, tr fetch.Transport) (*SyncReport, error) {
	...
	if remote.Auth.Token != "" {
		headers["Authorization"] = "Bearer " + remote.Auth.Token
	}
	req := fetch.Request{
		URL:          remote.URL,
	...
	for name, oid := range fr.Tips {
		...
		// Mark progress into request headers after each staged tip (nil-safe via HeaderMap).
		headers["X-Mirror-LastRef"] = name
